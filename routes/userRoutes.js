...
router.post("/register", async (req, res) => {
  ...
  if (req.body.password.length < 8 || req.body.password.length > 1024)
    return res
      .status(400)
      .json("Password must be between 8 and 1024 characters");
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(400).json({
        message: "Company Name not Found, please double check spelling",
      });
    }
    ...
    res.cookie("token", token, {
      httpOnly: true, // Prevents the token from being accessed by JavaScript
    });
