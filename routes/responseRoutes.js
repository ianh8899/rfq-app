...
router.post('/', async (req, res) => {
    try {
        ...
        const existingResponse = await ResponseRoutes.findOne({ rfq: req.body.rfq, createdBy: user._id });
        if (existingResponse) {
            return res.status(400).json({ message: 'You have already submitted a response for this RFQ.' });
        }
        ...
        if (rfq.status === 'Closed') {
            return res.status(400).json({ message: 'This RFQ is closed and cannot be responded to.' });
        }

        // Create a new response and save it
        const newResponse = new ResponseRoutes({ ...req.body, createdBy: user._id });
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const onlyStatusUpdate = Object.keys(req.body).length === 1 && req.body.status;
        if ((!user.buyer && onlyStatusUpdate) || (!onlyStatusUpdate && response.createdBy.toString() !== user._id.toString())) {
            return res.status(403).json({ message: 'Access denied' });
        }
