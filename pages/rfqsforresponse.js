        ...
        async function fetchRFQs() {
            ...
            try {
                ...
                for (let i = 0; i < fetchedRfqs.length; i++) {
                    const responseRes = await axios.get('http://localhost:5000/response/' + fetchedRfqs[i]._id, {
                        headers: {'Authorization': `Bearer ${token}`}
                    });
                    ...
                    const supplierResponse = responseRes.data.find(response => response.createdBy === localStorage.getItem('userId'));
                    if (supplierResponse) {
            ...
            <ul>
                {rfqs.map(rfq => (
                    <div key={rfq._id} className="rfq">
                        ...
                        <div className="rfq-item status" style={{borderColor: determineBorderColor(rfq.responseStatus)}}>
                            <p>{rfq.responseStatus}</p>
                        </div>
                        <button className="rfq-item" onClick={() => window.location.href = '/responsetorfq/' + rfq._id}>View</button>
                    </div>
                ))}
            </ul>
        </div>
