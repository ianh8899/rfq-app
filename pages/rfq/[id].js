...
const RfqDetails = () => {
    ...
    const fetchRfqAndResponses = async () => {
        ...
        const responsesResponse = await axios.get(`http://localhost:5000/response/${id}`, {headers: {'Authorization': `Bearer ${token}`}})

        // For each response, fetch the corresponding user to include username in the response data
        const responsesWithUsernames = await Promise.all(responsesResponse.data.map(async response => {
            const userResponse = await axios.get(`http://localhost:5000/user/${response.createdBy}`, {headers: {'Authorization': `Bearer ${token}`}});
            return {
                ...response,
    ...
    const submitChanges = async () => {
        ...
        await axios.put(`http://localhost:5000/rfq/${id}`, rfqUpdates, {headers: {'Authorization': `Bearer ${token}`}})
        setEditMode(false)
...
// Function to change the status of a response
    const changeResponseStatus = async (responseId, currentStatus) => {
        ...
        const token = localStorage.getItem('token');
        await axios.put(`http://localhost:5000/response/${responseId}`, {status: nextStatus}, {headers: {'Authorization': `Bearer ${token}`}})
        ...
        <div className="container">
            ...
            {editMode ? (
                <div className="edit-mode">
                    ...
                    <div>
                        <label>Status:</label>
                        <select name="status" value={rfqUpdates.status} onChange={handleInputChange}>
                    ...
                    <div>
                        ...
                        <input type="date" name="contractStartDate" value={rfqUpdates.contractStartDate}
                               onChange={handleInputChange}/>
                    </div>
                    <div>
                        <label>Annual Volume:</label>
                        <input type="number" name="annualVolume" value={rfqUpdates.annualVolume}
                               onChange={handleInputChange}/>
                    </div>
                    <div>
                        <label>Contract Length:</label>
                        <input type="number" name="contractLength" value={rfqUpdates.contractLength}
                               onChange={handleInputChange}/>
                    </div>
                    <div>
                        <label>About:</label>
                        <textarea name="about" value={rfqUpdates.about} onChange={handleInputChange}/>
                    </div>
                    <div>
                ...
                <>
                    ...
                    <button className="edit-button" onClick={toggleEditMode}>Edit RFQ</button>
                </>
            )}
            ...
            {responses.map(response => (
                <div key={response._id}>
                    <div className="response-grid">
                        <div className="pill"><p>Submitted by: {response.createdBy}</p></div>
                        ...
                        <div className={`pill status ${response.status.toLowerCase()}`}><p>Status: {response.status}</p>
                        </div>
                        <div className="pill"><p>Comments: {response.comments}</p></div>
                    </div>
                    <div className="change-response">
                        <button onClick={() => changeResponseStatus(response._id, response.status)}>Change Response
                            Status
                        </button>
                    </div>
                </div>
