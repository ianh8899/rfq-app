...
const OutstandingRFQs = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:5000/rfq', { headers: { 'Authorization': `Bearer ${token}` } })
        ...
        <div>
            ...
            {rfqs.map(rfq => (
                <div key={rfq._id} className="rfq">
                    ...
                    <div className="rfq-item status" style={{borderColor: rfq.status === 'Open' ? 'green' : 'red'}}>
                        <p>{rfq.status}</p>
                    </div>
                    <button className="rfq-item" onClick={() => router.push(`/rfq/${rfq._id}`)}>View</button>
                </div>
            ))}
        </div>
    );
